...
transformed data {
  ...
  vector[12] offset_probs_g1;
  vector[12] offset_probs_g2;
  ...
  if(do_offsets == 1 && do_grouping == 1){
    ...
    offset_probs_g1 = rep_vector(1, 12);
    offset_probs_g2 = offset_probs;
    
  } else {
    
    offset_probs_g2 = rep_vector(1, 12);
    offset_probs_g1 = offset_probs;
    N_offset_elems_g2 = 1;
  }
...
generated quantities {
  matrix [N_units, 12] betas;  // final vector of seasonality coefficients by admin unit
  ...
  for (i in 1:N_u_lower) {
    ts_prob[i]  = 0;
    for (j in 1:N_offset_elems) {
      for (k in 1:N_offset_elems_g2) {
        int off_uyears[N_u_lower] = offset_years[j, ]; // unique years
        int off_uyears_g2[N_u_lower] = offset_years[k, ]; // unique years
        
        real x = lambdas[uunits[i]] * (betas_g1[umonths[i]] + etas[off_uyears[i], 1]) + 
        do_grouping * (1-lambdas[uunits[i]]) * (betas_g2[umonths[i]] + etas[off_uyears_g2[i], N_eta_groups]) +
        convolved_re[uunits[i]] * sigma;
        
        ts_prob[i]  += inv_logit(x) * offset_probs_g1[j] * offset_probs_g2[k];
      }
    }
  }  
  ...
  for (u in 1:N_units) {
    for (i in 1:12) {
      if (do_grouping == 0) {
        betas[u, i] = betas_g1[i];
      } else {
        betas[u, i] = lambdas[u] * betas_g1[i] + (1-lambdas[u]) * betas_g2[i];
      }
    }
  }
  ...
  for (i in 1:N_obs) {
    ...
    for (j in 1:N_offset_elems) {
      for (k in 1:N_offset_elems_g2) {
        prob[i] += inv_logit(prob_non_zero[j,k][i]) * offset_probs_g1[j] * offset_probs_g2[k];
      }
    }
    if (prob[i] > (1 - 1E-10)) {
      prob[i] = 1 - 1E-10;
